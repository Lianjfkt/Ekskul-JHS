...
/**
 * Hook for importing student data in batches via Supabase upsert.
 *
 * Returns:
 *  { startImport, progress, result, isImporting, reset }
 *
 * `progress` – { current: number, total: number, label: string }
 * `result`   – { inserted: number, updated: number, failed: { row, nis, error }[] }
 */
export function useImportStudents() {
   ...
   *   The already-validated rows. `valid` = new rows, `updates` = rows that will update.
   * @param {Function} [onComplete] – called after all batches finish
   */
