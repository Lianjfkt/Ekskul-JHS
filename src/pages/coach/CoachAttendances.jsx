  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Pencatatan Kehadiran Siswa</h1>
          <p className="text-slate-500 text-sm">Pilih kelas ekskul dan sesi latihan, kemudian tandai status kehadiran setiap siswa.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
        ...
        <div className="space-y-1.5">
          ...
          <select
            ...
            className="flex h-11 w-full rounded-xl border border-input bg-white px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:bg-slate-50 disabled:text-slate-400"
          >
            {sessions.length === 0 ? (
      ...
      {selectedSession && students.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-100 text-center">
            <span className="text-2xs font-semibold text-emerald-600 uppercase">Hadir</span>
            <h4 className="text-2xl font-extrabold text-emerald-800 mt-1">{stats.hadir}</h4>
          </div>
          <div className="p-4 rounded-xl bg-amber-50 border border-amber-100 text-center">
            <span className="text-2xs font-semibold text-amber-600 uppercase">Izin</span>
          ...
          <div className="p-4 rounded-xl bg-rose-50 border border-rose-100 text-center">
            <span className="text-2xs font-semibold text-rose-600 uppercase">Alpha</span>
      ...
      {selectedSession ? (
        loading ? (
          <div className="p-8 text-center text-slate-500 font-medium">Memuat absensi siswa...</div>
        ) : students.length === 0 ? (
          <div className="p-8 text-center text-slate-400 bg-white rounded-xl border border-slate-100 shadow-sm flex flex-col items-center gap-2">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex justify-center gap-6">
                              {['hadir', 'izin', 'alpha'].map(status => (
                                <label key={status} className="flex items-center gap-1.5 cursor-pointer capitalize select-none text-xs font-medium text-slate-700">
                                  <input
                                    type="radio"
                                    ...
                                    className={`h-4 w-4 border-slate-300 focus:ring-primary ${
                                      ...
                                      status === 'izin' ? 'text-amber-600' : 'text-rose-600'
                                    }`}
                                  />
                                  <span>{status}</span>
                                </label>
