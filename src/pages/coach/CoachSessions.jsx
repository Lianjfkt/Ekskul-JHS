  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Jadwal & Log Sesi Latihan</h1>
          <p className="text-slate-500 text-sm">Catat setiap pertemuan latihan, materi/topik yang dibahas, dan catatan khusus.</p>
        </div>
      ...
      {/* Main Content */}
      {loading ? (
        <div className="p-8 text-center text-slate-500 font-medium">Memuat data sesi...</div>
      ) : managedEkskuls.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                  {sessions.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-6 py-8 text-center text-slate-400">Belum ada sesi latihan yang dibuat.</td>
                    </tr>
                  ) : (
                    sessions.map(session => (
                      <tr key={session.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-6 py-4 font-mono font-semibold text-slate-900 flex items-center gap-1.5 mt-2">
                        ...
                        <td className="px-6 py-4 font-bold text-slate-900">{session.extracurricular?.name}</td>
                        <td className="px-6 py-4 font-semibold text-slate-900">{session.topic || 'Sesi Umum'}</td>
                        <td className="px-6 py-4 text-slate-500 max-w-xs truncate">{session.notes || '-'}</td>
                        <td className="px-6 py-4 text-right space-x-2">
                          <Button onClick={() => handleOpenModal(session)} variant="ghost" size="icon" className="h-8 w-8 hover:bg-slate-100 hover:text-primary">
                            <Edit2 className="w-4 h-4" />
                          </Button>
                          <Button onClick={() => handleDelete(session.id)} variant="ghost" size="icon" className="h-8 w-8 hover:bg-destructive/10 text-destructive hover:text-destructive">
