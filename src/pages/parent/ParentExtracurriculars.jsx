...
export default function ParentExtracurriculars() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold text-slate-800">Ekskul Anak</h1>
        <p className="text-sm text-slate-500 mt-0.5">Semua ekstrakurikuler yang diikuti oleh {selectedChild.full_name.split(' ')[0]}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-12 text-center border border-violet-50 shadow-sm">
          ...
          <p className="text-sm text-slate-400">Anak Anda belum terdaftar di ekskul apapun.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {enrollments.map(enr => {
